...
class VisualBertEmbeddings(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids=None, visual_feats=None, visual_token_type_ids=None):
        seq_length = input_ids.size(1)
        position_ids = torch.arange(seq_length, dtype=torch.long, device=input_ids.device)
        position_ids = position_ids.unsqueeze(0).expand_as(input_ids)
        if token_type_ids is None:
        ...
        visual_embeddings = self.projection(visual_feats)
        visual_token_type_embeddings = self.token_type_embeddings_visual(visual_token_type_ids)
        visual_position_ids = torch.zeros(*visual_feats.size()[:-1], dtype = torch.long,device=visual_feats.device)
        visual_position_embeddings = self.position_embeddings_visual(visual_position_ids)
        
        
        visual_embeddings = visual_embeddings+visual_position_embeddings+visual_token_type_embeddings
...
class VBBertModel(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        ...
        self.apply(self.init_bert_weights)
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
                visual_feats=None, visual_token_type_ids=None,visual_attention_mask=None):
        ...
        embeddings = self.embeddings(input_ids, token_type_ids=token_type_ids,
                                           visual_feats=visual_feats, visual_token_type_ids=visual_token_type_ids)
        
        lang_v_feats = self.encoder(hidden_states = embeddings, attention_mask = extended_attention_mask)
        pooled_output = self.pooler(lang_v_feats)
        return pooled_output

class VBFeatureExtraction(BertPreTrainedModel):
    """
    BERT model for classification.
    """
    def __init__(self, config):
        ...
        """
        super().__init__(config)
        self.bert = VBBertModel(config)
        self.apply(self.init_bert_weights)

    def forward(self, input_ids, token_type_ids=None, attention_mask=None, visual_feats=None,
                visual_token_type_ids=None, visual_attention_mask=None):
        pooled_output = self.bert(input_ids,token_type_ids=token_type_ids,attention_mask=attention_mask,
                                  visual_feats=visual_feats,visual_token_type_ids=visual_token_type_ids,
                                  visual_attention_mask=visual_attention_mask)
        return pooled_output
...
class VBFeatureExtractionAtt(BertPreTrainedModel):
    """
    BERT model for classification.
    """
    def __init__(self, config):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, visual_feats=None,
                visual_token_type_ids=None, visual_attention_mask=True):
        return self.bert(input_ids,token_type_ids=token_type_ids,attention_mask=attention_mask,
                                  visual_feats=visual_feats,visual_token_type_ids=visual_token_type_ids,
                                  visual_attention_mask=visual_attention_mask, output_attentions=True)
...
class UniterOcrEmbeddings(nn.Module):
    def __init__(self, config):
        super().__init__()
    ...
    # def forward(self, ocr_feat, ocr_pos_feat, type_embeddings):
    #     transformed_im = self.ocr_layer_norm(self.ocr_linear(ocr_feat))
    #     transformed_pos = self.pos_layer_norm(self.pos_linear(ocr_pos_feat))        
    #     embeddings = transformed_im + transformed_pos + type_embeddings
    #     embeddings = self.LayerNorm(embeddings)
    #     embeddings = self.dropout(embeddings)
    #     return embeddings    
    
    def forward(self, ocr_feat, ocr_pos_feat):
        transformed_im = self.ocr_layer_norm(self.ocr_linear(ocr_feat))
        transformed_pos = self.pos_layer_norm(self.pos_linear(ocr_pos_feat))        
...
class UniterModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, txt_type_ids=None,attention_mask=None,
        ...
        img_embeddings = self.img_embeddings(img_feats, img_pos_feat, img_type_embeddings)
        embeddings = torch.cat([word_embeddings,img_embeddings],dim=1)
        lang_v_feats = self.encoder(hidden_states = embeddings, attention_mask = extended_attention_mask)
        pooled_output = self.pooler(lang_v_feats)
        return pooled_output

class UniterOcrModel(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.embeddings = UniterTextEmbeddings(config)
        self.img_embeddings = UniterImageEmbeddings(config)
    ...
    def forward(self, input_ids, txt_type_ids=None,attention_mask=None,
        ...
        img_type_embeddings = self.embeddings.token_type_embeddings(img_type_ids)
        img_embeddings = self.img_embeddings(img_feats, img_pos_feat, img_type_embeddings)
        ocr_embeddings = self.ocr_embeddings(ocr_feats, ocr_pos_feat)
        embeddings = torch.cat([word_embeddings,img_embeddings,ocr_embeddings],dim=1)
        lang_v_feats = self.encoder(hidden_states = embeddings, attention_mask = extended_attention_mask)
        pooled_output = self.pooler(lang_v_feats)
        return pooled_output
...
class ChexBertImageEmbeddings(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.img_linear = nn.Linear(1024, config.hidden_size)
        self.img_layer_norm = BertLayerNorm(config.hidden_size, eps=1e-12)
        #self.pos_layer_norm = BertLayerNorm(config.hidden_size, eps=1e-12)
        #self.pos_linear = nn.Linear(2, config.hidden_size)

        # tf naming convention for layer norm
        self.LayerNorm = BertLayerNorm(config.hidden_size, eps=1e-12)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)

    def forward(self, img_feat, type_embeddings):
        transformed_im = self.img_layer_norm(self.img_linear(img_feat))
        #transformed_pos = self.pos_layer_norm(self.pos_linear(img_pos_feat))        
        #embeddings = transformed_im + transformed_pos + type_embeddings
...
class ChexBertModel(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.embeddings = ChexBertTextEmbeddings(config)
        self.img_embeddings = ChexBertImageEmbeddings(config)
        self.encoder = BertEncoder(config)
        self.pooler = BertPooler(config)
        self.apply(self.init_bert_weights)
    def forward(self, input_ids, txt_type_ids=None,attention_mask=None,
                visual_attention_mask=None,img_feats=None,img_type_ids=None):
        attention_mask = torch.cat((attention_mask,visual_attention_mask),dim=1)
        extended_attention_mask = attention_mask.unsqueeze(1).unsqueeze(2)
        ...
        word_embeddings = self.embeddings(input_ids, txt_type_ids)
        img_type_embeddings = self.embeddings.token_type_embeddings(img_type_ids)
        img_embeddings = self.img_embeddings(img_feats, img_type_embeddings)
        embeddings = torch.cat([word_embeddings,img_embeddings],dim=1)
        encoder_outputs = self.encoder(hidden_states = embeddings, attention_mask = extended_attention_mask)
        lang_v_feats = encoder_outputs[0]
        pooled_output = self.pooler(lang_v_feats)
        return pooled_output


class ChexBertFeatureExtraction(BertPreTrainedModel):
    """
    BERT model for classification.
    """
    def __init__(self, config):
        ...
        """
        super().__init__(config)
        self.chexbert = ChexBertModel(config)
        self.apply(self.init_bert_weights)

    def forward(self, input_ids, token_type_ids=None, attention_mask=None, visual_feats=None,
                visual_token_type_ids=None, visual_attention_mask=None):
        pooled_output = self.chexbert(input_ids,txt_type_ids=token_type_ids,
